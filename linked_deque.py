...
class LinkedDeque:
    def __init__(self, initial_data: Iterable|None =None, front_or_back: str ='Back') -> None:  # required
        # self.front = None
        # self.back = None  # In Java one might use self.clear [this centralizes methods to decrease size of testing path]
        self.clear()  # Works, here, no warnings, in PyCharm, I think this would result in linting for use of self._____ not declared in __init__(...)
        #             # as I'm writing up documentation for this, I think I prefer declaring these explicitly in init
    ...
    def __getitem__(self, index: int) -> any:  # O(N^2) for accessing all data_portions by index
        # item = self.front.data  # .copy? deepcopy (for objects) ? I guess it depends. [probably better not to copy (can happen client-direction from this, if needed, and adding it here would prevent actually using the objects to make changes to their containing data structures)]
        # for g in range(len(self)):  # recursive traversal for index, passing a list, resulting in a Generator?
        #     if g == index % len(self):
        #         item = self.front.data
        #     self.front_to_back()
        # return item  ### This would actually be slower to use: O(N^2) for each index use in a for each statement
        ...
        if isinstance(other, self.__class__):
            if len(self) == len(other):
                ...
                for e_i in range(len(self)):
                    ...
                    if eq_bool:
                        break
                while self.get_front() != front_was:  # relies on __eq__ method of data portions
                    self.front_to_back()
        return eq_bool
    ...
    def add_to_back(self, new_entry) -> None:  # required
        if new_entry is not None:  # These handle null entries so that methods in client classes can send null entries w/o adding DLNode(data_portion=None)
            if not isinstance(new_entry, LinkedDeque.DLNode):  # These might be slowing down methods that use them, maybe add a private method that moves nodes as nodess
    ...
    def remove_back(self) -> any:  # required
        back_node = self.back                           # TODO further: replace this line with assignment of data_portion, comment-out line setting back_node previous to None
    ...
    def display(self) -> None:  # required
        # display_string_list = []
        # for each in self._items_data_list():
        #     display_string_list.append(str(each))
        # print("LinkedDeque: " + ', '.join(display_string_list))
        # # another option: pass a string through a recursive call that goes through the deque
    ...
    def _display_recursive(self, in_string: str, current_node) -> str:
        if current_node is None:
            ...
            return self._display_recursive(in_string + (', ' if current_node is not self.front else '') + str(current_node.data_portion), current_node.get_next_node())
