...
class LedgerEntry:
    ...
    def add_purchase(self, new_purchase: StockPurchase) -> None:  # required  # O(1)
        # add to the back
    ...
    def add_purchase_front(self, new_purchase: StockPurchase) -> None:  # O(1)
        if new_purchase.symbol == self.symbol:  # this is twice as many primitive operations per call of this method than without validation
    ...
    def remove_purchase(self) -> StockPurchase:  # required  # O(1)
        ...
        return self._linked_deque.remove_front()#.get_data_portion()  # DLNode is designed as a private inner class of LedgerEntry, so this doesn't return DLNode objects, instead it returns their data portion
    ...
    def position_entry_add_between(self, cost_per_share: float) -> None:  # O(N)
        # positions entry such that addition of shares to back of queue keeps queue in sorted order (ascending)  ### assumes that the queue is in sorted (ascending) order
        for b_i in range(len(self)):
            if (cost_per_share <= self._linked_deque.get_front().cost) and (cost_per_share >= self._linked_deque.get_back().cost):
    ...
    def align_head_tail(self) -> None:  # O(N)
        for a_i in range(len(self)):
            if self._linked_deque.get_front().cost < self._linked_deque.get_back().cost:
                break  # assuming the deque is in ascending order, the lowest cost shares are at the front
            else:
                self.increment_entry()
    ...
    def median(self) -> float:  # O(N^2) = O(N) + O(N) * O(N)
        ...
        for m_i in range(ledger_length):  # O(N) outer
            ...
            num_equal = 0
            if not m_i == 0:
                self.increment_entry()  # increment entry to data portion to check for median
            # deque position j
            current_cost = self.peek().cost
            if median is None:
                current_cost = self.peek().cost
                for m_j in range(ledger_length):  # O(N) inner  # count each incremented front value
                    if self.peek().cost > current_cost:  # using count_ methods of LinkedDeque, this would go through the deque 3 times
                        num_greater += 1
                    elif self.peek().cost < current_cost:
                        num_lesser += 1
                    else:
                        num_equal += 1
                if abs(num_lesser - num_greater) <= num_equal:  # condition from codestepbystep solution  ### for 1 node, 0 - 0 <= 1
                    median = current_cost                  #    # if the data were sorted then the current_cost would be the median
            # position j
            # loop ends with position j = position i + (length - 1)
        self.increment_entry()  # runs in the case when length is 1, but this doesn't change the order  # could be in an else statement of the for loop, but behavior would be the same
        # deque position i
