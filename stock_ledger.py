...
class StockLedger:
    ...
    def equals(self, other) -> bool:
        ...
        # return str(self) == str(other)  # This takes into account abstraction of instance fields for equivalence, but, as-is, it ignores a lot ('is' might be a better comparison)
    ...
    def open_sale(self, stock_symbol: str, quantity: int, price: float) -> tuple[StockSale|None, LedgerEntry|None]:  # O(number of shares)
        ...
        if sell_entry is None:
            print(f"Stock symbol not found for {quantity} shares of {stock_symbol}")  # maybe instead of these, have client code making sense of returned values including some error code
            sale = None
    ...
    def buy(self, stock_symbol: str, shares_bought: int, cost_per_share: float) -> None:  # required  # O(shares_bought)
    ...
    def buyRandom(self, stock_symbol: str, shares_bought: int, cost_per_share: float) -> None:  # O(N) = O(N) * O(quantity)
        buy_entry = self.open_entry(stock_symbol)
        for s_i in range(shares_bought):  # O(shares_to_buy) * O(len(buy_entry))
            for s_j in range(SystemRandom().randint(0, len(buy_entry) - s_i)):  # O(N)
                buy_entry.increment_entry()
            if SystemRandom().random() >= 0.5:
                buy_entry.add_purchase(StockPurchase(stock_symbol, cost_per_share))
            else:
                buy_entry.add_purchase_front(StockPurchase(stock_symbol, cost_per_share))

    def buyOptimal_1(self, stock_symbol: str, shares_bought: int, cost_per_share: float) -> None:  # O(N)
    ...
    def buyOptimal_2(self, stock_symbol: str, shares_bought: int, cost_per_share: float) -> None:  # O(N^2)
        ...
        if len(buy_entry) == 0:
            ...
            if cost_per_share >= buy_entry.median():  # idea: interesting design problem, chain together one or more pairs of buy and sell methods (do by instance field assigned None or object) but not all of them
                for share in range(shares_bought):
                    buy_entry.add_purchase(StockPurchase(stock_symbol, cost_per_share))
            else:
                for share in range(shares_bought):
    ...
    def buyOptimal_3(self, stock_symbol: str, shares_bought: int, cost_per_share: float) -> None:  # O(1) = O(shares bought)
    ...
    def sell(self, stock_symbol: str, quantity: int, price: float) -> StockSale:  # required  # O(1) = O(1) * O(quantity)
    ...
    def sellRandom(self, stock_symbol: str, quantity: int, price: float) -> StockSale:  # O(N)
        ...
        if sale is not None:
            ...
            for s_i in range(quantity):  # O(quantity)
                for s_j in range(SystemRandom().randrange(1, sell_entry_length * 2)):  # O(N) = O(2N)
                    sell_entry.increment_entry()
                sale.add_sale(sell_entry.remove_purchase())  # removes from front  # O(1)
        return sale

    def sellOptimal_1(self, stock_symbol: str, quantity: int, price: float) -> None:  # O(N)
        """sellOptimal_1 sells the lowest cost shares first"""
        sale, sell_entry = self.open_sale(stock_symbol, quantity, price)
        while not sale.is_filled():  # O(N) = O(N) * quantity  # fill the sale
            if sale.quantity - len(sale) == self.number_of_shares(stock_symbol):  # O(N)
                ...
                lowest_cost_share = sell_entry.peek()  # O(1), gets a StockPurchase, in this case
                ...
                while not sale.is_filled() and sell_entry.peek().cost == lowest_cost_share.cost:
                    sale.add_sale(sell_entry.remove_purchase())
        return sale

    def sellOptimal_2(self, stock_symbol: str, quantity: int, price: float) -> None:  # O(N)
        """sellOptimal_2 sells any shares up to the "median" cost (per iteration, calculated as (maximum - minimum) / 2 + minimum)"""
        sale, sell_entry = self.open_sale(stock_symbol, quantity, price)  # number of shares >= quantity
        if sale is not None:
            while not sale.is_filled():  # this loop will not run indefinitely (number of shares >= quantity)
                current_median = sell_entry.median_2()  # O(N)
                for t_j in range(len(sell_entry)):  # must visit up to each data_portion
                    if sale.is_filled():  # order is filled
                        break
                    if sell_entry.peek().cost <= current_median:     # examine a purchase
                        sale.add_sale(sell_entry.remove_purchase())  # add to sale, remove from front
                    else: 
                        sell_entry.increment_entry()                 # or add to back of deque
        return sale
    
    def sellOptimal_3(self, stock_symbol: str, quantity: int, price: float) -> None:  # O(1)
        """sellOptimal_3 sells the lower cost share from the front or back"""
        sale, sell_entry = self.open_sale(stock_symbol, quantity, price)  # number of shares >= quantity
        if sale is not None:
            while not sale.is_filled():  # this loop will not run indefinitely (number of shares >= quantity)
                if sell_entry.peek() > sell_entry.peek_back():  # StockPurchase has __lt__ method
                    sell_entry.decrement_entry()
                sale.add_sale(sell_entry.remove_purchase())
        return sale
    ...
    def contains(self, stock_symbol: str) -> bool:  # required  # O(number of entries)
        for each_entry in self.ledger_entries:
            if each_entry.symbol == stock_symbol:
                return True
        return False

    def get_entry(self, stock_symbol: str) -> LedgerEntry:  # required  # O(number of entries)
        for each_entry in self.ledger_entries:
            if each_entry.symbol == stock_symbol:
                return each_entry
        return None
    ...
    def number_of_shares(self, stock_symbol: str) -> None|int:  # O(N)
        ...
        if self.contains(stock_symbol):
            num_shares = len(self.get_entry(stock_symbol))  # returns None for non-present stock_symbols
            # self.get_entry(stock_symbol)._linked_deque.count_eq_data_portion(StockPurchase(stock_symbol))  # Another idea for how to accomplish this
        return num_shares  # same behavior for stock symbols not found in ledger and for those of empty ledger entries
